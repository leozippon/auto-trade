...
function stepTreePanel(detail) {
  ...
  // A tree that cannot be read leaves the host empty rather than the pane in
  // an error state: the session's own panels below it are unaffected.
  api(`/api/experiments/${id}/steps`)
    .catch(() => ({ nodes: [] }))
    .then((tree) => {
...
/* Depth-first walk of one lineage, emitting each row with the rails that
   ...
   which children are visible and which subtrees are collapsed. */
function walkLineage(roots, childrenOf, emit, collapsedOf) {
