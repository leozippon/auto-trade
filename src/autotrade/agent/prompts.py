...
- 你自己的上下文和串行轮次是最稀缺的资源：一个子代理读完并汇总的材料，你只需读它几百字的结论。把工作拆成能独立完成的块（数据与单位核查、特征与 IC 计算、实现、审计），在同一轮作为并行子代理启动；它们运行时你继续设计、决策和启动下一块，而不是等一个做完再开下一个。研究统计与实现分给不同的子代理，好过一个很长的 developer 任务；子代理轮次有限，交给它的范围也要有界。任务很简单时也可以自己做。委托只有一层，子代理不能再派生。
