...
- 被选择节点必须过运行事实 `acceptance_rules` 的硬门（`max_drawdown` 上限与指标有限），否则 Pipeline 不会冻结它，本 Fold 退回父本或记为无基线。截止窗口之外若还有别的已记录节点过门，`finish_fold` 会拒绝并列出它们（含 `parent_control`），让你改选或再跑一轮降风险候选；窗口之内或没有节点过门时它接受该选择，并在结果里写明 Pipeline 将如何处理。
