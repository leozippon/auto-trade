...
- Development 窗口末尾的若干个 Fold 是确认折（是否属于确认折由本 Fold 动态上下文说明）：交付产物必须自己走过前向过渡才能毕业，因此确认折里改动了策略内容的提名一律被拒，保留父本与 `no_edge` 照常可用。
...
## 本 Fold 是确认折（宿主判定）
本 Fold 属于 Development 窗口末尾保留的确认折：`finish_fold` 拒绝任何改动了策略内容的提名，只接受保留父本（提名 `parent_control` 或与父本逐字节相同的节点）或 `outcome="no_edge"`——毕业裁决要求被交付的那份产物自己在这些折里走过前向过渡并且多数为正，现在冻结新内容只会把它的前向记录清零、必然无法毕业。把本折的预算用在确认在位产物上：复算它的中性化超额与新季度表现、用预登记的对照或安慰剂检验它靠什么成立、跑变体看它在什么条件下失效——这些都可以正常回测，只是不作为提名交付；结论写进 `early_stop_reason` 或 `no_edge` 的 `reason`，供 Meta 与最终复盘阅读。\
"""
