...
- 冻结门在提名时执行，规则与阈值在 `acceptance_rules.freeze_gate`：节点是完整研究期验证、指标有限、研究期权益回撤与主动回撤都不超过上限，本臂至少两个完整研究期验证，提名节点的主动信息比率、它去膨胀后的夏普概率（DSR）与主动超额为正的研究年数都过线；本臂设有跟踪授权（`tracking_mandate`）时，节点自身对沪深300的跟踪误差与市场 β 还要落在限内——那是约束，不是技能的证据。试验数是本臂验证过的全部不同 revision，任何 span、任何尝试、对照都算。不过门的提名被拒并给出命名原因与读数，会话继续；试验数只增不减，搜索越多，冻结需要的证据越强。
- 一条臂至多冻结一次，冻结后没有第二次机会：前推期不过，本臂就结束了。所以冻结的是逐年一致、比过对照、按毕业条件（`acceptance_rules.graduation`：前推期主动中性化超额的下界、最近半年、两种回撤、成本压力、交易活跃度与跟踪授权）设计的节点。被冻结的是节点的不可变快照，工作副本不必先恢复到它。
