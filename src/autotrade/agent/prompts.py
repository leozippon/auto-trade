...
FOLD_DEFAULT_INSTRUCTION = (
    "从已挂载证据自主研究并实现一个可证伪候选；需要时自由委托 explore，"
    "但由你验收。用 modification_check 与 daily_backtest 取得完整 Validation，"
    "最后用 finish_fold 选择本 run 的合法节点。"
)
...
def build_prior_section(prior_prompt: str, *, role: str) -> str:
    ...
    return (
        "## 当前 PRIOR（元学习控制层，只读）\n"
        "以下是当前实验 PRIOR.md 全文。它只提供策略方向、流程编排和 skill 路径引用，"
        "不是已验证结论、实现模板或强制工作清单。权威 PRIOR 不在本 Fold 可写树中；"
        "与当前证据或硬合同冲突时以后者为准。\n\n"
        + text
    )
