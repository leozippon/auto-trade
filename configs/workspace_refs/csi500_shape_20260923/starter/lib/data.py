...
which keeps a 500-name index in the same order of magnitude as a 300-name one
(measured: about 2.5x, still a hundredth of the fit budget).

Nothing in this module names an index. The one place a benchmark code is written
down is `lib/index.py`, and the run facts' `benchmark_index` is what makes it
correct -- see that module and `pit-field-map.md`.
