...
the rows visible at the decision; `daily` already carries `daily_basic`, whose
`turnover_rate` (stamped 18:00 of its session) is a DECIMAL in the snapshot
(0.05 = 5 % of the float traded that day). A full-day suspension has no row, so
it is a missing session, never a zero. The book is sized at the RAW T-1 close.
"""
...
DAILY_COLUMNS = ["ts_code", "trade_date", "close", "turnover_rate"]
UNIVERSE_COLUMNS = ["ts_code", "name", "l1_name"]
# Calendar days read: LONG sessions plus weekends and the longest holidays.
LOOKBACK_DAYS = int(knobs.LONG * 7 / 5) + 60
...
def panel(context, held):
    ...
    if len(dates) < knobs.LONG:
        raise RuntimeError(f"only {len(dates)} sessions in the daily window; the score needs {knobs.LONG}")
...
def tradable(panel):
    """(names,) bool: in the newest section, with a T-1 bar, not ST (and never STAR / BSE)."""
