...
1. 读本轮 `data_summary.json`、`unit_reference.json`、daily schema 与 `runtime_env.json`。确认列名、单位与包版本（`lightgbm`、`torch`）。在会话沙箱 `shell` 里运行 `nvidia-smi` 与 `python -c "import torch; print(torch.cuda.is_available())"`，记下 GPU 是否真的可见，并从 `runtime_env.json` 的 `sandbox_spec` 读出本臂的 `gpu_count`：同一个值也决定正式回放的策略容器有没有 GPU。
