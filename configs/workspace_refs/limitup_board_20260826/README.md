...
开发只有一个长 Validation 窗口（没有 Test 阶段，也没有 Epoch 循环），输入窗是它之前约 21 个月，可用的完整 Validation 次数远多于机制数量（上限以本轮事实为准）。先在输入窗上把多个 cohort 的事件研究做完，再预先登记最强的三条机制，用 `batch_validate` 并列跑完整 Validation，并按窗口内的子区间判稳健。
