...
Horizon N = the leg's horizon (`knobs.HORIZON` for every leg but `f3`). The
window runs from the open of t+1 to the open of t+1+N; the book itself sells
at 09:30 and buys at the 15:00 close of t+1, a mismatch whose cost the round's
census measured as nil (`sources.md`).
...
This label is the carrier's and every leg keeps it; the arm moves only how the
book follows the score. The one registered exception is `f3`, which keeps the
same construction of the label and only matches its horizon to the holding
period the fresh book actually realises (`families.md`).
"""
