...
**开工第一件事**：核对三件事——`macro.datasets` 里有没有 `index_weight`（没有它就没有这个臂的股票池）与 `index_daily`（没有它就没有残差标签）；`universe` 带不带 `l1_name`（P2 的行业敞口与 `f2` 的行业拉回）；以及运行事实里的 **`benchmark_index`** 与 `lib/index.py` 的 `INDEX_CODE` 是否一致（不一致时改常量去对齐运行事实，反过来不行）。**缺任何一块都不得静默回退**到别的池子、别的标签或别的特征集再按同一套口径报数——那是另一个登记，按 `families.md` 的终止规则诚实终止或改登记。
...
**只有月末截面，这是这一面最大的限制。** 两张截面之间的真实调整、临时剔除与停牌成分都看不见；复核日拿到的是上一张截面的名单（方向上保守，细节见 `references/index-membership.md`）。`c_base` 的月度复核与截面同频：自然月首个决策日刚好有一张新截面，新纳入的名字在这一次进书、被剔除的在这一次强卖。`f1` 按周桶复核，**与截面不同频**：一个月里几次复核用的是同一张截面，一张新截面要到它之后的第一次复核才起作用（按三周桶最多晚约三周），离开成分的持仓也要到那一次复核才强卖。这是 `f1` 与 `c_base` 的一处构造差，结果笔记里要报每次复核的强卖只数。
...
| `daily` | `circ_mv` | **元** | 同上 | 只给 `f2` 的规模暴露（对数流通市值）用；分数与其余腿不读它 |
