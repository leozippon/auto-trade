...
# --- block ---------------------------------------------------------------
# Rolling windows of the flow block, in trading days.
FLOW_SHORT = 5
FLOW_LONG = 20
# A stock-day whose session produced fewer than FLOW_MIN_NRET signed minutes,
# or whose board was sealed, carries a queue artefact rather than order flow.
# Both are masked to NaN before any rolling window (`pit-field-map.md`).
FLOW_MIN_NRET = 200
FLOW_DROP_SEALED = True
# Calendar days of the events read. Must cover FLOW_LONG trading days plus
# the warm-up the fit window needs; holidays make the ratio worse than 7/5.
FLOW_LOOKBACK_DAYS = 120
