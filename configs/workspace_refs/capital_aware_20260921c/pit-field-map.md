...
| 开局 | 读哪里 | 陷阱 |
|---|---|---|
| 质量 overlay（盈利 / 应计 / 投资） | `fundamentals` 三张主报表与 `fina_indicator_vip`。盈利用归母 `n_income_attr_p`，不要 `n_income`。应计从利润与经营现金流的差来，投资从资产或资本开支的变化来 | 报表金额是元、年初至今累计；`fina_indicator_vip` 的比率是百分数。取首版还是时点生效版写进 `hypothesis`。不要 EP/BP |
| percent accrual | 同上，percent accrual =（盈利 − 经营现金流）相对资产 | 经营现金流在 `cashflow_vip`。覆盖从研究首年春季才够做同比 / TTM，第一个研究年单列汇报 |
| 52 周高比 | `daily` 的收盘与 `adj_factor`，用复权价取约 252 个交易日的高点 | `adj_factor` 不是逐日非降（见数据文档里个别代码的跳变）。未复权高点不是 52 周高。这不是动量、也不是反转 |
| 月度 −MAX（跳过近 5 日） | `daily` 复权收益。MAX 取复核窗口内日收益的最大，**跳过最近 5 个交易日** | 不是 20 日反转，也不是日频价量堆。跳过近 5 日是为了不把刚发生的大涨当天算进去 |
| 成分内反彩票 overlay | 同上，分数在沪深 300 成分内打 | 证伪条件对着 20 日反转的逐日秩相关。相关度高说明它不是新信息 |
| 拆股调整净发行 | 总股本的拆股调整变化（`daily_basic` 的股本列，用 `adj_factor` 还原拆股）。低发行更好 | **不要把 `events.share_float_complete` 的解禁排序当主分数。** `float_ratio` 是单笔解禁占总股本的百分数，多数行小于 1、中位约 0.0003%，按小数读会选错集合 |
