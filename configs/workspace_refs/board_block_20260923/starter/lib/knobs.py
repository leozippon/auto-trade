...
# --- block ---------------------------------------------------------------
# Rolling windows of the board block, in trading days. The long window carries
# the recent-activity columns, the extra one the rare-event columns: a
# constituent hits a limit on roughly 0.2-1.2 % of its days, so a 20-bar
# window alone would be zero for most names on most bars.
BOARD_LONG = 20
BOARD_EXTRA = 60
# Calendar days of the events read. Must cover BOARD_EXTRA trading days plus
# the warm-up the fit window needs; holidays make the ratio worse than 7/5.
BOARD_LOOKBACK_DAYS = 120
