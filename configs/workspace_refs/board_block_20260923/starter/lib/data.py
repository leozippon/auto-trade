...
UNADJUSTED turnover in CNY: it is the denominator the block's listed-flow
ratios divide by, and an adjusted one would not match the exchange's own
listed amounts.

Candidates. `c_base` is the carrier alone -- the 158 operators and nothing
else. `b1` is the carrier plus the block. `b_only` is the block alone, which is
...
from lib import board as block, features, index, knobs
...
def block_coverage(panel, values):
    """Per-bar share of the bar's constituents whose block is active (`lib/board.py`)."""
