"""Entry: the confirmed Alpha158 + LightGBM carrier, with a limit-board block bolted on.
...
`CANDIDATE` selects what this package is; `families.md` is the authority:
    ...
    "b1"       the same code with the 17-column board block appended to the same
    ...
    "b_only"   the block alone, no Alpha158 columns. A diagnostic: it answers
...
monthly book (lib/trade.py). The block itself is lib/board.py.
