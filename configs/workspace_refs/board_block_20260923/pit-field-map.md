...
- **这是一条刀刃**：默认 `inference_time` 也是 08:30，判可见的比较是 `available_at <= inference_at`，所以 T-1 的榜单**正好落在边界上、可见**。**第 0 轮必须先确认运行事实里的 `inference_time` 确实是 08:30**；如果它更早，本块就少掉最近一天的榜单，这是一个要如实报出来的读数差，不是一个可以忽略的细节。本块的四组列全是 20 日与 60 日窗口的聚合，所以少一天不改结论，但**报要报**。
...
- **数据下限**：研究期第一个决策视图里 `daily` 回溯五年、`index_weight` 从 2020-01 起、本块的四张表也从 2020-01 起，所以三年训练窗口在第一个研究年里是完整的；但第一个研究年的读数仍要单独汇报。
