...
**开工第一件事**：核对 `macro.datasets` 里有没有 `index_weight` 与 `index_daily`，以及 `universe` 带不带 `l1_name`。前两个决定能不能建指数内的书与残差化的标签，第三个是**行业敞口那条要求**与可选的标签行业去均值的字段。**缺任何一块都不得静默回退**到别的池子、别的标签或别的基准再按同一套口径报数——那是另一个机制，按 `families.md` 的终止规则诚实终止或改登记。
...
**训练面板上的成员资格按同一条规则逐日重建**：训练日 t 的截面 = 日期不晚于 t 的最新一张。起步包的 `panel._membership` 就是这条规则，它同时保证了标签、特征 z 分数与注意力的 token 集合是同一个集合。

另一面是修正：该表每个开市日重取当年分区，供应商一旦回写历史权重，改后的数值会带着原来的 `available_at` 进入历史快照。幅度未量化，与宏观整段替换同类。

## 基准自己的日线 `macro.index_daily`

标签的基准腿与市场门控的状态都从这里读，**不从成分股重构指数**。
...
- **一个已接受的口径缺口**：`universe` 整表是决策日（回放中是该槽锚点）的一个口径，所以训练窗口里每一天用的是同一份行业标签与同一份名字。起步包因此**只在决策行上做 ST/退与板块的可交易筛选，不把它推回训练窗口**——把今天的 ST 标签推回两年前，等于系统性地删掉后来变坏的名字，那是前视。行业标签没有这个退路（P2 的敞口汇报与可选的标签去均值都要它），它是本包已知的、量级最小的一处点内不一致。
...
研究期第一个决策视图里，`fundamentals`、`macro` 与 `events` 从 2020 年初开始，`daily` 回溯五年，`index_weight` 与 `index_daily` 同样从 2020-01 起。**两年训练窗口在第一个研究年的起点上刚好够**（训练窗口要往前再吃 260 个自然日的算子与序列窗口热身）：第一个研究年的 `fit` 训练日数会比后面的年份少，读数单列汇报，不要把第一年的快与省内存外推到整期。
...
- **归一化不能跨时点。** 特征的截面 z 分数是逐日算的，市场状态用的是**固定尺度**而不是在面板上拟合出来的均值方差——拿整段窗口去估一组归一化常数再套回每一天，是一次安静的前视，而且会让训练日与决策日的口径不同。
- **截面只有 300 个名字**。全市场每个截面 2,400–3,500 个名字，成分只有 300 上下：同样的训练年数下样本少一个量级，`fit` 会快很多、内存也低，但**过拟合的风险高很多**，而注意力的参数量比一个 GRU 还多。加容量（序列窗口、头数、隐藏维、层数、特征列）之前先看 G-INC 与逐年读数，不要先加容量再找理由。
