...
`index_daily` carries the benchmark's own open and close, stamped 17:30 of its
trade date like the sections. The label's benchmark leg comes from it, so the
strategy never reconstructs an index from its members.
...
# another is a `refs/families.md` termination condition.
