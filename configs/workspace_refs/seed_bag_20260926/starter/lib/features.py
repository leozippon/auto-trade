"""Alpha158 feature block (158 operators) - Qlib's public Alpha158 operator set, this repository's own vectorised implementation (provenance in `refs/sources.md`).
...
window d in {5,10,20,30,60}). Qlib does not fix a canonical order; this one is the
lineage's, and `data.FEATURE_NAMES` pins it for every fit and every score.
"""
