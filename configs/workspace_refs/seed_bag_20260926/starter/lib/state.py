...
The counter lives here, not in a model file, because every booster of every
leg must count refits on exactly the same schedule: each seed of a bag
continues its own booster file on this cadence and is cold on the same
refit, so a bag member is the booster its seed trains alone, file for file,
and a difference between two legs is the seeds they average.
