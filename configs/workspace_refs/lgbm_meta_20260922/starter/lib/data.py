"""The one panel builder: Alpha158 inputs over the CSI 300 constituent union, for fit and for decisions.

`wide(context, calendar_days)` reads the window once and returns (names x dates)
matrices; `features(...)` turns a slice of them into the 158 Alpha columns,
cross-sectionally robust-z-scored over the constituents of each bar. `fit` and a
review call the same two functions on the same window definition, so the matrix
a booster is trained on and the matrix it scores are built identically.

The name axis is the UNION of every constituent of every visible section in the
window, not the whole market. That is the one change to the graduated lineage's
data path that this arm makes on purpose: the Alpha158 + LightGBM primary has
only ever been fitted on the all-market sub-30-CNY pool, where a zero-skill book
of the same shape read -4.67 %/yr, and it is being moved onto the shape the host
panel now grades against. Reading only the union also keeps the fit small: about
420 names and 820 bars for a three-year window instead of 3,500 names.
...
CNY, `circ_mv` CNY, `turnover_rate_f` a decimal, `index_daily` prices index
points.
"""
...
DAILY_COLUMNS = ["ts_code", "trade_date", "open", "high", "low", "close", "vol", "amount",
                 "adj_factor", "turnover_rate_f", "circ_mv"]
...
def wide(context, calendar_days):
    ...
    panel = {
        ...
        "circ_mv": dense(frame["circ_mv"].to_numpy(dtype=np.float64)),
        "turn_f": dense(frame["turnover_rate_f"].to_numpy(dtype=np.float64) * 100.0),
