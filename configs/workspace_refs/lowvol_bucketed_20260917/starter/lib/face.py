"""PIT-safe construction of the low-residual-volatility face and its registered controls.
...
CANDIDATES = {
    "s_lb": "lowvol",        # main candidate: ivol60 ranks, bucketed selection
    "c_uncapped": "lowvol",  # control: the same score and pool, no buckets
    "c_rand": "random",      # control: the same buckets, a fixed random score
...
# An arbitrary but permanently fixed constant: `c_rand` is only a control while
# its draw never moves, neither between legs of a batch nor between rounds.
...
def _random_score(codes):
    ...
    Drawing once per code rather than once per decision makes the random book a
    stable 15 names that only turns over when a name leaves the pool -- the
    bucketing control has to hold the buckets fixed and the ranking
    uninformative, not add turnover the candidate does not have. The same seed
    and the same table give the same value in a cold worker, a warm worker and
    any later round.
    """
...
def build_cross_section(context, candidate):
    ...
    if kind == "lowvol":
        ...
        raw = _random_score(section["ts_code"])
...
def neutralize(section, candidate):
    ...
    The two score-free legs have no face to residualize -- residualizing an even
    spread or a random draw turns a book that carries no information into a book
    of whatever the residual happens to rank -- so they keep their raw score and
    their ranking stays uninformative.
    """
    if kind_of(candidate) in ("pool", "random") or len(section) < 30:
