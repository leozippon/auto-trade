...
| 频率 | 每指数每月一张，日期是当月最后一个交易日，2020-01 起无缺口；中证 500 每张恒为 500 只 |
...
- **用最新一张截面标注历史决策日**：中证 500 在研究期末视图的 60 张截面里在册过 922 个名字，漏掉的历史成分多半是跌出去的（或升进了沪深 300）。
- **把停牌日当成振幅为零的一天**：停牌日没有日线；按日历取 20 天的窗会把停牌前后的名字算得更「安静」。起步包按每只股票自己的最近 20 根取窗。
- **一手与整手**：100 万元 50 席上一手买不起的成分不到 1 %（`refs/sources.md`）；买不起只影响买入，**已持有的名字不得因此强卖**；买单按最近整手四舍五入再按现金截断。
