"""Entry: the confirmed Alpha158 + LightGBM carrier, with an intraday price-path block bolted on.
...
`CANDIDATE` selects what this package is; `families.md` is the authority:
    ...
    "s1"       the same code with the 15-column intraday-statistics block appended to the same
    ...
    "s_only"   the block alone, no Alpha158 columns. A diagnostic: it answers
...
monthly book (lib/trade.py). The block itself is lib/istats.py.
