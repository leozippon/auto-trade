...
from raw values, where the factor cancels by construction. The block is built
from its own dataset (`lib/istats.py`) and reads nothing from this panel except
its axes and the membership mask.

Candidates. `c_base` is the carrier alone -- the 158 operators and nothing
else. `s1` is the carrier plus the block. `s_only` is the block alone, which is
...
from lib import features, index, istats as block, knobs
