...
The shape is deliberately plain, because this arm is about the learner and not
...
name among that entry day's names on the original's side of constituent
membership that this money can buy, so it aligns the pool, the calendar and the
money per seat -- but it does not align a weighting scheme.
...
year instead of ~52. The cost is registered and real: the label's horizon is
`knobs.HORIZON` bars, so a monthly review acts on a score whose horizon has
already expired twice over, and the same lineage's note warns that a monthly
review of a short-horizon score can read near zero. That tension is this
construction's main risk, it is written into `families.md` as a named kill
condition rather than discovered later, and `MAX_SWAPS` is sized so the book can still move at the
registered seat count. Round 0 measures turnover and cost on a replay instead
of deriving them from this paragraph.

Sizing. At CNY 1m over the registered seats a seat is around CNY 19,400, so
one 100-share lot of 95-96 % of the constituents fits and affordability stops
being the price factor it is on a small account -- which is exactly why a learner change is
priced here and not at 100k. A name's money is `book value / seats`, in
100-share lots rounded to the NEAREST lot and then clipped to the cash on hand;
rounding down would put a systematic quarter of a seat back in cash.
...
Ties. A lambdarank booster stops after very few trees (`sources.md`), so its
scores take few distinct values and whole groups of names tie at the seat
cut. The head packs broke ties by `ts_code`, which is harmless on a continuous
score and a board tilt on a tied one (SZ main board and ChiNext codes sort
before SH codes). Ties here are broken by a fixed hash of the code: stable
across decisions, so a tie cannot churn the book, and unrelated to board or
exchange. On a score without ties -- `c_base` -- the book is unchanged.
"""
...
from lib import data, index, knobs, primary
...
def run(context, candidate):
    ...
    score = primary.score(context, panel, candidate)
    ...
    meta.update(primary.report(context, candidate))
