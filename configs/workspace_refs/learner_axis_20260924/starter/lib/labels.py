...
prices a learner on a fixed carrier, and moving the label at the same time
would make "what changed" unanswerable. The lambdarank legs do not get a new
label either: `grades` cuts THIS rank target into integer relevance grades,
because a ranking objective needs ordinal labels, and every leg is scored
against the same continuous target.
"""
...
def grades(target):
    """Integer relevance grades 0..knobs.RANK_GRADES-1 of a rank target in [-0.5, 0.5).

    Equal-width cuts of the per-bar rank, so every grade holds the same share
    of a bar's names and the top grade is the top 1 / RANK_GRADES of the bar.
    """

    cut = np.floor((np.asarray(target, dtype=np.float64) + 0.5) * knobs.RANK_GRADES)
    return np.clip(cut, 0, knobs.RANK_GRADES - 1).astype(np.int32)
