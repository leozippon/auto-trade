"""PIT-safe construction of the borrowed low-residual-volatility face.
...
This module is the sibling pack `lowvol_bucketed_20260917`'s face, and this
arm does not register any of it: the pool, the ranking column and the
neutralization are a fixed borrowed component, so that the exclusion screens of
`lib.screens` are the only free variable. The three edits are the candidate
table below, the removal of that pack's `c_rand` scorer (no leg here uses it)
and making the visibility helper public, since `lib.screens` reads under the
same rule.

The ranking face is low 60-day residual volatility:
...
The quality columns the construction screens on are built here too, from the
FIRST announced version of the latest statement (income_vip, cashflow_vip,
balancesheet_vip joined on (ts_code, end_date), report_type "1", CNY, YTD flows
annualised by the quarter number); a statement older than STATE_MAX_AGE trading
days carries no signal:

    accr    = (n_income_attr_p - n_cashflow_act) / total_assets
    ocf_ta  = n_cashflow_act / total_assets * 4 / quarter
    circ_mv = float market cap, the column the pool floor and the float-cap
              terciles sort on

`industry` is the as-of SW L1 industry name carried by the universe view --
membership in force at the decision day, SW2014 before the Shenwan index switch
and SW2021 after it. It is the bucket key of `lib.trade` AND the key the
score-level neutralization dummies use, and it is spelled exactly as the host's
own attribution spells it, unclassified names included (UNCLASSIFIED), so that
the book's bucket counts and the reported largest-industry weight are the same
quantity.
...
# differs between the pre-registered legs of one batch; `lib.trade.SCREENS` says
# which names each of them excludes and `lib.trade.SHAPES` how it fills the book.
CANDIDATES = {
    "s_scr": "lowvol",        # main candidate: the borrowed book behind three exclusion screens
    "c_noscreen": "lowvol",   # control: the same book, no screen at all
    "c_placebo": "lowvol",    # control: the same book behind a permanent random exclusion
...
STATEMENT_LOOKBACK_DAYS = 700 # calendar days of statements read per decision
...
def visible_rows(frame, context):
    """Rows whose row-level `available_at` is at or before the decision moment.

    `lib.screens` reads three more panels of the same two domains and judges
    visibility by exactly this rule, so it lives here once.
    """
...
def read_universe(context):
    """ts_code, listing name and date, and the as-of SW L1 industry name."""
...
def read_index_returns(context):
    ...
    frame = visible_rows(frame, context)
...
def read_statements(context):
    ...
    frame = visible_rows(frame, context).copy()
...
def statement_values(statements):
    """One row per statement: the screening column and the yoy growth control column."""
    ...
    # `yoy_np` is a neutralization column only. Same caliber as the sibling pack:
    # cumulative profit against the statement four rows earlier for the same name.
...
def daily_context(daily):
    ...
    lottery face, and `ep`/`yoy_np` keep the leg from turning into value or
    growth. Dropping any of them changes the mechanism, not just the noise.
    """
...
def _pool_spread(size):
    ...
    carries no information about the leg. Descending order puts those
...
def build_cross_section(context, candidate):
    ...
    `ivol60`, `accr` and `circ_mv` for the construction, `close` for lot sizing
    and `industry` for the buckets. Names without a live statement
    (age <= STATE_MAX_AGE) or without the volatility window are excluded, for
    every leg alike: the pool is the same for the candidate and its controls or
    their readings do not compare.
    """
    ...
    industry = universe.set_index("ts_code")["l1_name"].reindex(section["ts_code"])
    section["industry"] = industry.fillna(UNCLASSIFIED).replace("", UNCLASSIFIED).to_numpy()
...
def neutralize(section, candidate):
    ...
    The score-free leg has no face to residualize -- residualizing an even
    spread turns a book that carries no information into a book of whatever the
    residual happens to rank -- so it keeps its raw score and its ranking stays
    uninformative.
    """
    ...
    dummies = pd.get_dummies(section["industry"].astype(str), drop_first=True, dtype="float64")
