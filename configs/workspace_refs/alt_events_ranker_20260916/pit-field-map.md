...
| `events.report_rc` | `ts_code`、`report_date`、`create_time`、`org_name`、`author_name`、`quarter`、`eps`、`np`、`rating`、`report_type`、`classify` | `eps` 元/股；`np`/`op_rt`/`op_pr`/`tp` 万元（`tp` 是**利润总额**不是目标价）；`pe`/`ev_ebitda` 是券商**前瞻**倍数；`rd` 是预测**股息率**（百分数）不是研发费用；`quarter` 是**财年**标签（98.8% 以 `Q4` 结尾，`2024Q4` = FY2024），但约 0.3% 的行是脏值：1,532 行只有 `'Q'`、3,449 行为 NULL，`astype(float)` 会直接抛 `ValueError`，必须 `pd.to_numeric(..., errors="coerce")`；切片里没有 `report_title`/`name`/`imp_dg` |
