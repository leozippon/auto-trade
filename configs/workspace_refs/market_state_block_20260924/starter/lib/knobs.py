...
Groups, in the order a session usually touches them:
    ...
    block       the market-state columns this arm is here to price
...
# --- block ---------------------------------------------------------------
# Trailing windows of the market-state block, in trading days: the short one
# for returns, realized vol, the near moving average and dispersion; the long
# one for the far moving average, the rolling high, the vol ratio's
# denominator, breadth and the turnover z-score. STATE_LONG must not exceed
# the carrier's warm-up (`data.WARMUP_BARS`): the benchmark series starts
# 2020-01-02, and `market_state.build` raises on any state missing on a usable
# bar rather than letting a gap mark the early dates (`pit-field-map.md`).
STATE_SHORT = 20
STATE_LONG = 60
