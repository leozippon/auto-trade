"""The one panel builder: the carrier's Alpha158 inputs, the market-state block, and their union.
...
matrices are built identically -- the single most common way a two-part
...
previous trading day (T-1). The constituent sections and the benchmark series
carry `available_at` and are filtered by it (`lib/index.py`).
...
else. `m1` is the carrier plus the twelve date-level state columns, `m_int` the
carrier plus the twelve state x stock interactions (`lib/market_state.py`).
There is no block-only leg: the twelve state columns alone cannot order the
names of a bar at all, and the interactions alone are three stock factors
re-signed by the state, which is not the question this arm asks.

The block is NOT z-scored across the bar like the carrier's columns are. A
state column is one value per bar, so a cross-sectional z-score would zero it,
and an interaction's z-score would collapse to sign(state) x z(stock); the
block arrives on scales that are already comparable across bars. A missing
interaction enters as the neutral 0, the carrier's own convention for a
missing operator.
"""
...
from lib import features, index, knobs, market_state as block

WARMUP_BARS = 60               # leading bars that only warm the operators and the state windows
...
def block_coverage(panel, values):
    """Per-bar share of the bar's constituents whose block columns are all finite (`lib/market_state.py`)."""
...
def build_block(panel, candidate):
    return block.build(panel, candidate, WARMUP_BARS)
...
def feature_names(candidate):
    ...
    if candidate == "m1":
        return CARRIER_NAMES + list(block.STATE_NAMES)
    if candidate == "m_int":
        return CARRIER_NAMES + list(block.INTERACTION_NAMES)
...
def wide(context, calendar_days):
    ...
    panel["bench_close"] = pd.to_numeric(benchmark["close"], errors="coerce").reindex(
        pd.Index(dates)).ffill().to_numpy()
    panel["bench_amount"] = pd.to_numeric(benchmark["amount"], errors="coerce").reindex(
        pd.Index(dates)).ffill().to_numpy()
...
def _new_block(values, keep):
    """(len(keep), names, K) float32 block columns for the bars in `keep`, unscaled; missing is 0."""
...
def _matrix(panel, values, lo, hi, keep, candidate):
    carrier = _carrier_block(panel, lo, hi, keep)
    if candidate == "c_base":
        return carrier
    return np.concatenate([carrier, _new_block(values, keep)], axis=2)
