...
任务只有一句话：在「决策时可见的最近一个转债交易日仍有存续转债的正股」这个约 300–500 只的股票池上，检验五个以转债市场为信息源、能用决策时可见的 `cb_daily`/`cb_basic`/`cb_call` 重算的正股多头假说家族——转债残差动量、转股溢价率状态、强赎条款前后的正股行为、转债相对正股的成交额异动、纯债溢价率的债底缓冲——每个家族带同池等权对照与匹配的无转债对照，只做正股多头、不交易转债，并按本包的「没有边际」读法如实淘汰。前几批六臂全部在同一只证券的价量、事件、文本上做变换，这是第一条用同一发行人的另一只证券做信息源的臂；`cb_*` 三个数据集此前 refs=0、PRIOR=0、策略代码=0。
...
- 转债行情与日线同步、强赎公告晚一天：`cb_daily`（以及 `index_daily` 等当日发布的宏观日频表）的行级 `available_at` 在快照里是 `trade_date` 当日 17:30（规则名 `contract_1730_from:trade_date`，与 `daily` 同一收盘合同），随当晚的晚间落库节点放行，因此回放里 08:30 决策拿得到 T-1 的转债行情，与日线一致；`cb_call` 的行级 `available_at` 仍是 `ann_date` 当日 23:59:59，晚于按前一个工作日 23:35 放行的节点开始时刻，最新只拿得到 T-2 的强赎公告（长假后的首个交易日才重新看到节前最后一日）。`cb_call` 只有本折第一个决策日例外——冻结首片按锚点 23:59:59 收行，带着锚点当天的公告。信号与存续判断按 T-1 对齐，强赎事件窗按 T-2 对齐，不要假定 T-1 公告。
...
- 存续与转股价只能从 `cb_daily` 重算：`cb_basic.conv_price`/`remain_size`/`newest_rating`/`delist_date` 是当前状态、已被快照剔除；历史转股价 `conv_price_t = 100 × 正股close_t / cb_value_t`，存续按 T-1 有无 `cb_daily` 行判断（见上一条的可见边界）。
...
账户是 10 万元真实资金：佣金万一、最低 5 元/笔，过户费 0.1 bp，卖出印花税按成交日切换（切换前万十，切换后万五），方向滑点 5 bp。单只 7,000–10,000 元时最低佣金就是 5–7 bp/边，一次完整换仓的往返成本约 25–35 bp；5 日持有的家族一年要付几十次往返，超额必须显著大于成本才算数。100 股整手：股价 40 元的一手占单只预算四成以上，本包把 T-1 收盘 ≤ 40 元写成声明过滤；科创板 200 股起、北交所 100 股起，通常买不起一手，本池科创板占 8%、无北交所，默认剔除科创板并说明。默认 `inference_time=08:30`：当日日线、`daily_basic`、`adj_factor` 都不可见，只能用 T-1 及更早，转债行情与 `index_daily` 同样到 T-1，`cb_call` 公告只能用 T-2 及更早；成交只有决策日当天 09:30 开盘价与 15:00 收盘价两个时点（相对 T-1 数据日是次日；`execute_at` ≥ 决策时刻即可）。
