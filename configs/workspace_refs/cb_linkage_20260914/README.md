...
- 转债字段比日线晚一天可见：`cb_daily` 与 `cb_call` 的行级 `available_at` 是 `trade_date`/`ann_date` 当日 23:59:59，而整个 macro 域按前一个工作日 23:35 的晚间落库节点放行，因此回放里 08:30 决策最新只拿得到 T-2 的转债行情与 T-2 的强赎公告（日线仍到 T-1；长假后的首个交易日才重新看到节前最后一日）。只有本折第一个决策日例外——冻结首片按锚点 23:59:59 收行，带着锚点当天的转债行。信号、存续判断与强赎事件窗一律按 T-2 对齐，不要假定 T-1 转债数据。
...
- 存续与转股价只能从 `cb_daily` 重算：`cb_basic.conv_price`/`remain_size`/`newest_rating`/`delist_date` 是当前状态、已被快照剔除；历史转股价 `conv_price_t = 100 × 正股close_t / cb_value_t`，存续按 T-2 有无 `cb_daily` 行判断（见上一条的可见边界）。
...
账户是 10 万元真实资金：佣金万一、最低 5 元/笔，过户费 0.1 bp，卖出印花税按成交日切换（切换前万十，切换后万五），方向滑点 5 bp。单只 7,000–10,000 元时最低佣金就是 5–7 bp/边，一次完整换仓的往返成本约 25–35 bp；5 日持有的家族一年要付几十次往返，超额必须显著大于成本才算数。100 股整手：股价 40 元的一手占单只预算四成以上，本包把 T-1 收盘 ≤ 40 元写成声明过滤；科创板 200 股起、北交所 100 股起，通常买不起一手，本池科创板占 8%、无北交所，默认剔除科创板并说明。默认 `inference_time=08:30`：当日日线、`daily_basic`、`adj_factor` 都不可见，只能用 T-1 及更早，转债与其余 macro 域只能用 T-2 及更早；成交只有决策日当天 09:30 开盘价与 15:00 收盘价两个时点（相对 T-1 数据日是次日；`execute_at` ≥ 决策时刻即可）。
