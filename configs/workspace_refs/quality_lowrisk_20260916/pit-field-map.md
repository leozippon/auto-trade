...
先核对本轮 `data_summary.json`、snapshot manifest 与 `unit_reference.json`：`fundamentals.datasets` 必须列出 `income_vip`、`cashflow_vip`、`balancesheet_vip`（缺任一，排名分数整块不可构造，本臂没有候选，只能诚实终止）；`macro.datasets` 应列出 `index_daily`（缺它时 `ivol60` 按登记变体改用总波动，买单元数据 `vol_basis` 写 `total`，结果笔记必须写明）。本文是本包可见时点、单位、去重规则与陷阱的**唯一权威表述**。
...
- **`pct_chg` 两种单位**：`macro.index_daily.pct_chg` 是百分数（`0.68` = 0.68 %），`daily.pct_chg` 是小数；把指数直接当小数用会把 β 压到零、残差波动等于总波动而不报错，本包的二次排序就会排在一个错的列上。
...
- **总资产可以是零或空**：新上市或异常行按下限截断后仍参与，但 `accr`/`ocf_ta` 任一为空即无排名分数，该名字不入池。
- **`is_suspended` 只标盘中临停**，整天停牌的股票在 `daily` 里没有行；用最新交易日有无行判断可交易。
- **整手与价格上限**：100 股整手，T-1 收盘价 ≤ 30 元使一手 ≤ 3,000 元；科创板 200 股起，本包剔除。
- **构造的顺序是机制的一部分**：池底 → 排名 → 二次排序 → 取前 15。先二次排序再截池底、或先取前 15 再截池底，都不是本包登记的候选。
- **对照共用同一个池**：`c_q15`、`c_ivol`、`c_pool` 与候选的股票池必须逐字相同，只差成形与分数；池一变，读数就不可比。
