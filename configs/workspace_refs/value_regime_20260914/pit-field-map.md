...
| `macro.index_daily` | `000300.SH`/`000905.SH`/`000852.SH` 的 `close`（指数点）、`pct_chg`（百分数，5% 存成 5.0） | T-1 可见；仓库只落七只核心指数，覆盖自 2020 年起 |
| `macro.fut_daily` | `ts_code`、`close`/`settle`（指数点）、`vol`/`oi`（手）、`oi_chg` | `conservative_date_eod`，T-1 可见。CFFEX 的 `IF/IH/IC/IM` 每日各 8 行：4 个实际月合约 + `X.CFX`/`XL.CFX`/`XL1`/`XL2`/`XL3` 连续代码 |
...
| `macro.opt_daily` | `ts_code`、`close`/`settle`（权利金）、`vol`/`oi`（张） | T-1 可见。SSE 自 2015-02-09、SZSE 与 CFFEX 自 2019-12-23；只落金融期权（ETF + 股指），无商品期权 |
