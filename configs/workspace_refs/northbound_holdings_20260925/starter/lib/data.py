"""The one panel builder: the constituent universe, the book's prices, and the carrier's inputs.

`wide(context, calendar_days)` reads the window once and returns (names x dates)
matrices. Every candidate's review calls it, because the universe, the prices
the book is sized at and the industry labels come from it; only the carrier
legs (`c_base`, `n2`) go on to build Alpha158 columns from it
(`fit_samples` / `decision_features`), and they build the training matrix and
the decision matrix with the same functions in the same column order.
...
window instead of 3,500 names, which is what keeps the carrier's fit inside the
container.
...
are filtered by it (`lib/index.py`); so are the holder tables (`lib/holdings.py`).

Adjustment. Prices and volume are forward-adjusted to each symbol's newest
visible bar (`adj_factor / adj_factor of that bar`); every Alpha158 operator is
a ratio inside its own window, so the anchor cancels and only split
consistency matters. VWAP = amount / vol is built from raw values. The book is
sized at the RAW T-1 close.
"""
...
def wide(context, calendar_days):
    ...
    # The benchmark is a leg of the carrier's label, so a date without it is
    # not a usable training date. Clip the panel to the benchmark rather than
    # widening the read: widening would hand the first section's names to
    # earlier dates.
...
def decision_features(panel):
    """(names, 158) float32 carrier features of the newest visible bar."""

    last = len(panel["dates"]) - 1
    lo = max(0, last - CONTEXT - 5)
    return _matrix(panel, lo, last + 1, [last])[0]
...
def fit_samples(panel, target, realized):
    ...
    for start in range(0, total, stride):
        ...
        if wanted:
            matrix = _matrix(panel, lo, end, wanted)
