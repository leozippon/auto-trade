...
Only the carrier legs (`c_base`, and `n2` through it) train on this label; the
holdings score `n1` is not learned and never reads it. It is copied unchanged
from the carrier's lineage so that `c_base` here is the register's `c_base`.
"""
