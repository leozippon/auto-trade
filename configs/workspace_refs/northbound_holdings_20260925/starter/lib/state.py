...
The counter lives here, not in a model file, because the two legs that read
the carrier -- `c_base` and `n2` -- must reset on exactly the same refits;
otherwise `n2` minus `c_base` would price the chain as well as the holdings.
The holdings legs `n1` and `c_shuf` never fit, so they never touch it.
"""
