"""LightGBM cross-sectional ranker: the borrowed score, trained in fit, read on a review day.

Mechanism: objective=regression on the per-bar label rank - 0.5, feature_fraction=0.8,
bagging_fraction=0.8, bagging_freq=1, min_data_in_leaf=200, lambda_l2=10, num_threads=8;
validation = the last VALID_DAYS training bars after an EMBARGO_DAYS gap (lib/common.py);
600 rounds with early stopping 50. The grid is the single point (31, 0.05) -- the point
the direction study of this lineage measured and the point the wider grid selected on
every refit of that pack's smoke; among the points within 5% of the minimum validation
L2 the lexicographically smallest (num_leaves, learning_rate) still wins, so widening
the grid back is one constant. Each refit trains one new booster and replaces the
previous one.

This module is a borrowed, fixed component: this arm registers the fill, not the score.
Touching it is a different arm.
...
FIXED_PARAMS = {
    ...
    "lambda_l2": 10,
    "verbose": -1,
    # Pin the thread count: lgbm's default (auto) reads the host's core count, not the
    # container's cpu quota, and oversubscribing it is slowest exactly when the host is
    # already carrying the other replays of the batch.
    "num_threads": 8,
}
...
def _train_grid(Xtr, ytr, Xva, yva, names, seed=None, num_boost_round=NUM_ROUNDS,
    ...
    """The grid + embargoed early stopping + 5% plateau / lexicographic selection.
...
def fit(context, candidate):
    ...
    A leg whose score is not the learned one has nothing to train: rather than burn a
    fit container on a booster no review will read, it must declare no `fit` at all, and
    this raises if that edit was forgotten.
    """
    if not common.trains(candidate):
        raise RuntimeError(
            f"{candidate} ranks on {common.LEGS[candidate][0]} and trains nothing: "
