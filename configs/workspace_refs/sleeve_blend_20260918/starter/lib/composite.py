"""PIT-safe cross-section of sleeve A: four rank legs, size-neutralised, on a float-cap-floored pool.
...
Pool floor (the one construction rule this sleeve is registered with): after the
qualification screens, the bottom CAP_FLOOR of the REMAINING section by T-1 float
cap is dropped. The rank is computed inside the already screened section, never on
the whole market -- the two cuts are not the same cut, and the whole-market variant
was measured to be far weaker.

Score = mean of the leg ranks named in LEGS, then the residual of an OLS on
[1, rank(log circ_mv)] (size-neutral).

The section also carries two columns nothing here uses. `industry` is the decision
day's SW L1 name from `universe` (empty rows fall in "未分类") -- the same column the
result's `stats.benchmark.top_industry_weight` is computed from, so a candidate can
read or bound its own industry concentration without rebuilding the reader. `ivol` is
the residual volatility the low-vol leg already computes, carried out so the registered
inverse-volatility weighting variant costs no second pass over the daily window.
...
MIN_LISTED_DAYS = 120
CAP_FLOOR = 0.30              # drop the bottom this fraction of the SCREENED pool by T-1 float cap
...
def build(context):
    """(scored pool frame with ts_code, industry, ivol and score, T-1 close of every visible name).
    ...
    # In-pool float-cap floor: drop the bottom CAP_FLOOR of the QUALIFIED pool by
    # T-1 circ_mv. rank(pct=True) is computed inside this section only (PIT-safe, the
    # same visibility as the filters above), and ties share the same pct rank, so
    # equal circ_mv names are cut or kept together. Computing the same quantile on
    # the whole-market cross-section BEFORE the screens is a different, much shallower
    # cut, and it is not this sleeve's rule.
    section = section[section["circ_mv"].rank(pct=True) > CAP_FLOOR].reset_index(drop=True)
    if len(section) < 30:
        return None, closes
    ...
    return section[["ts_code", "industry", "ivol", "score"]], closes
