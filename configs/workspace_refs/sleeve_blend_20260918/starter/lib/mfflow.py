...
Evaluation (perf, semantics unchanged): per-symbol row arrays (dedup in the
same canonical order, last occurrence wins) and the daily (do, dmv, damt)
arrays are precomputed once in numpy; each query time then gathers at most
_W20 window rows by searchsorted offsets (one deduped row per trade_date, and
the audited availability rule makes no row with trade_date >= T visible at the
08:30 decision, so the 20 calendar-day window holds at most _W20 rows). All
window sums run over the gathered rows in od-ascending order with out-of-window
rows contributing 0.0, and every element-wise formula and NaN/denominator
condition is applied unchanged, so the outputs match the reference (bit-exact
except numpy reduction-tree last-ulp noise on the window sums, absorbed by the
downstream float32 RobustZScore).
"""
...
def _build_maps(frame, dframe):
    """Per-symbol numpy structures, keyed by ts_code string (all sorts stable,
    file order preserved on ties). Returns (mf_map, day_map) with:
      mf_map[ts]  (od (n,) i64 ascending, av (n,) i64, net (n,) f64,
                   elg (n,) f64, lg (n,) f64, okd (n,) bool, e (n,) f64,
                   e2 (n,) f64): same-(ts, trade_date) dedup (canonical
                   (trade_date, available_at, file order), last occurrence
                   wins) plus the same-day daily-bar element
                   e = net x 1e4 / circ_mv (NaN when missing / <= 0) and e2
      day_map[ts] (do (nd,) i64 ascending, dmv (nd,) f64, damt (nd,) f64)
    """
    day_map = {}
    if dframe is not None and not dframe.empty:
        dts = dframe["ts_code"].to_numpy()
        do = dframe["date_ord"].to_numpy()
        dmv = dframe["circ_mv"].to_numpy(dtype=np.float64)
        damt = dframe["amount"].to_numpy(dtype=np.float64)
        du, dc = np.unique(dts, return_inverse=True)
        o = np.lexsort((do, dc))  # (symbol, date_ord), stable
        do_s, dmv_s, damt_s, c_s = do[o], dmv[o], damt[o], dc[o]
        st, _i, sz = np.unique(c_s, return_index=True, return_counts=True)
        for i in range(len(st)):
            b0 = _i[i]
            day_map[du[st[i]]] = (do_s[b0:b0 + sz[i]],
                                  dmv_s[b0:b0 + sz[i]],
                                  damt_s[b0:b0 + sz[i]])

    mf_map = {}
    rows = np.nonzero(frame["dataset"].to_numpy() == DS_MF)[0]
    if rows.size:
        ts_all = frame["ts_code"].to_numpy()[rows]
        od = frame["trade_ord"].to_numpy()[rows]
        av = frame["avail_s"].to_numpy()[rows]
        net = frame["net_mf_amount"].to_numpy(dtype=np.float64)[rows]
        elg = (frame["buy_elg_amount"].to_numpy(dtype=np.float64)[rows]
               - frame["sell_elg_amount"].to_numpy(dtype=np.float64)[rows])
        lg = (frame["buy_lg_amount"].to_numpy(dtype=np.float64)[rows]
              - frame["sell_lg_amount"].to_numpy(dtype=np.float64)[rows])
        du_mf, dc = np.unique(ts_all, return_inverse=True)
        o = np.lexsort((av, od, dc))  # (symbol, trade_date, available_at), stable
        od_s, av_s, net_s, elg_s, lg_s, c_s = od[o], av[o], net[o], elg[o], lg[o], dc[o]
        st, _i, sz = np.unique(c_s, return_index=True, return_counts=True)
        for i in range(len(st)):
            b0, b1 = _i[i], _i[i] + sz[i]
            ob, ab = od_s[b0:b1], av_s[b0:b1]
            nb, eb, lb = net_s[b0:b1], elg_s[b0:b1], lg_s[b0:b1]
            ...
            _, fi = np.unique(ob, return_index=True)
            keep = np.concatenate((fi[1:] - 1, np.array([b1 - b0 - 1], dtype=np.int64)))
            od2, av2 = ob[keep], ab[keep]
            net2, elg2, lg2 = nb[keep], eb[keep], lb[keep]
            d = day_map.get(du_mf[st[i]])
            if d is not None and len(d[0]) > 0:
                do, dmv, _damt = d
                mi = np.searchsorted(do, od2, "left")
                mi_c = np.clip(mi, 0, len(do) - 1)
                match = (mi < len(do)) & (do[mi_c] == od2)
                cmvd = np.where(match, dmv[mi_c], np.nan)
            else:
                cmvd = np.full(od2.size, np.nan)
            okd = np.isfinite(cmvd) & (cmvd > 0.0)
            e = np.where(okd, net2 * 1e4 / np.where(okd, cmvd, 1.0), np.nan)
            mf_map[du_mf[st[i]]] = (od2, av2, net2, elg2, lg2, okd, e, e ** 2)
    return mf_map, day_map


def _evaluate_maps(maps, symbols, t_arr):
    """Evaluate the 8 feature columns at query times t_arr (int64 epoch
    seconds) from precomputed per-symbol maps.

    maps: (mf_map, day_map) from _build_maps; symbols: ordered symbol list;
    ...
    (m = 1) and the per-block fit evaluation (m = block bars, bars
    independent). Window-empty / no-row cells are NaN; RobustZScore turns
    them into 0 AFTER the cross-sectional median/MAD is computed. Each query
    gathers its window rows by searchsorted offsets (see the module note on the
    <= _W20 bound and the exactness guard below); the window sums and every
    NaN/denominator condition are the reference ones."""
    ...
    for si, ts in enumerate(symbols):
        ...
        if d is not None and len(d[0]) > 0:
            ...
            do = dmv = damt = None
            cmv = np.full(m, np.float64(np.nan))
            nd = 0
        # The fixed-K gather is exact iff (a) no row outside the 20 slots is
        # visible at any query (the reference window has no od upper bound,
        # so at the 08:30 decision every visible row has od < T_ord and the
        # window holds at most 20 deduped rows), (b) the reference mask
        # products are NaN-safe on the rows the gather drops: the reference
        # multiplies EVERY deduped row by its window mask and NaN * False ==
        # NaN, so a NaN in net/elg/lg (mf rows) or amount (daily rows) outside
        # the gathered slots poisons the reference sums but not the gathered
        # ones, and (c) the daily window [T-20, T) holds at most 20 bars (one
        # per day). Otherwise this symbol falls back to the reference (n x m)
        # mask - bit-identical formulas to the original implementation.
        need_exact = True
        if n2 > 0:
            amin_suf = np.minimum.accumulate(av[::-1])[::-1]
            lo0 = np.searchsorted(od, T_ord - _W20, "left")
            k0 = lo0 + _W20
            need_exact = bool(((k0 < n2)
                               & (amin_suf[np.clip(k0, 0, n2 - 1)] <= t_arr)).any())
        if not need_exact and n2 > 0:
            # Rows gathered at every t are exactly [lo_max, lo_min+20): a
            # NaN net/elg/lg elsewhere is mask-multiplied (NaN*False==NaN) by
            # the reference sums but absent from the gathered sums.
            lo_min = np.searchsorted(od, T_ord.min() - _W20, "left")
            lo_max = np.searchsorted(od, T_ord.max() - _W20, "left")
            out = np.ones(n2, dtype=bool)
            out[lo_max:min(lo_min + _W20, n2)] = False
            need_exact = bool((out
                               & (np.isnan(net) | np.isnan(elg) | np.isnan(lg))).any())
        if not need_exact and nd > 0:
            lo_min_d = np.searchsorted(do, T_ord.min() - _W20, "left")
            lo_max_d = np.searchsorted(do, T_ord.max() - _W20, "left")
            out = np.ones(nd, dtype=bool)
            out[lo_max_d:min(lo_min_d + _W20, nd)] = False
            need_exact = bool((out & np.isnan(damt)).any())
        if not need_exact and nd > 0:
            lo_d = np.searchsorted(do, T_ord - _W20, "left")
            hi_d = np.searchsorted(do, T_ord, "left")
            need_exact = bool(((hi_d - lo_d) > _W20).any())
        if need_exact:
            # reference window quantities (original implementation's formulas)
            ...
            n5f = f5.sum(axis=0)
            n20f = f20.sum(axis=0)
            se5 = (np.where(f5, e[:, None], 0.0)).sum(axis=0)
            se2_5 = (np.where(f5, e2[:, None], 0.0)).sum(axis=0)
            se20 = (np.where(f20, e[:, None], 0.0)).sum(axis=0)
            se2_20 = (np.where(f20, e2[:, None], 0.0)).sum(axis=0)
        else:
            # window rows: od-ascending gathered block starting at the first
            # row with od >= T_ord - _W20 (exactness checked above)
            lo = np.searchsorted(od, T_ord - _W20, "left")
            idx = lo[:, None] + slots[None, :]
            inb = idx < n2
            ii = np.clip(idx, 0, n2 - 1)
            odg = od[ii]
            w20 = inb & (av[ii] <= t_arr[:, None])
            w5 = w20 & (odg >= (T_ord - _W5)[:, None])
            c5 = w5.sum(axis=1)
            c20 = w20.sum(axis=1)
            netg = net[ii]
            s5net = (netg * w5).sum(axis=1)
            s20net = (netg * w20).sum(axis=1)
            s5elg = (elg[ii] * w5).sum(axis=1)
            s20lg = (lg[ii] * w20).sum(axis=1)
            pos20 = (w20 & (netg > 0.0)).sum(axis=1)
            if nd > 0:
                lo_d = np.searchsorted(do, T_ord - _W20, "left")
                idxd = lo_d[:, None] + slots[None, :]
                indb = idxd < nd
                iidi = np.clip(idxd, 0, nd - 1)
                dw = indb & (do[iidi] < T_ord[:, None])
                amt20 = (damt[iidi] * dw).sum(axis=1)
            else:
                amt20 = np.zeros(m, dtype=np.float64)
            f5 = okd[ii] & w5
            f20 = okd[ii] & w20
            ...
            e1 = e[ii]
            e2g = e2[ii]
            se5 = (np.where(f5, e1, 0.0)).sum(axis=1)
            se2_5 = (np.where(f5, e2g, 0.0)).sum(axis=1)
            se20 = (np.where(f20, e1, 0.0)).sum(axis=1)
            se2_20 = (np.where(f20, e2g, 0.0)).sum(axis=1)
        # ---- common tail: 8 feature columns from the window quantities ----
        ...
        std5 = _pstd(se5, se2_5, n5f)
        std20 = _pstd(se20, se2_20, n20f)
...
def _evaluate(frame, dframe, symbols, t_arr):
    """frame/dframe entry (decision path + one-shot fit panels): build the
    per-symbol maps once, then _evaluate_maps. frame: normalized moneyflow
    frame (read_mf output, may be empty); dframe: normalized daily frame
    (read_daily output, may be empty); symbols: ordered symbol list; t_arr:
    (m,) int64 epoch seconds. Returns the same {feature: (m, S)} mapping as
    _evaluate_maps (all-NaN when the moneyflow frame is empty)."""
    t_arr = np.asarray(t_arr, dtype=np.int64)
    if frame is None or frame.empty:
        return {n: np.full((len(t_arr), len(symbols)), np.nan, dtype=np.float64)
                for n in FEATURES}
    return _evaluate_maps(_build_maps(frame, dframe), symbols, t_arr)
