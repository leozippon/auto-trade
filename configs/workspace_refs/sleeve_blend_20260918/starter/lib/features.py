"""Alpha158 feature block (158 operators) - formulas strictly follow refs/alpha158.md.

Layout: every series is a (S, T) wide matrix (S symbols x T bars, trade_date ascending).
Rolling math runs along the date axis (axis=1, inner-contiguous) with strided window
views (sliding_window_view); there are no per-symbol Python loops. Outputs are (S, T)
matrices aligned so that out[:, b] uses bars b-d+1..b (inclusive). Bars without enough
history are NaN; the cross-section fill is done later by robust_zscore (missing -> 0).

Column order ALPHA158 (9 K-line + 4 position + 29 rolling x 5 windows, grouped by
window d in {5,10,20,30,60}, operator order as in the reference table). The reference
does not fix a canonical order; this one is the dev package's canonical choice.
"""
...
ALPHA158 = list(KLINE) + list(POS) + [f"{n}_{d}" for d in WINDOWS for n in ROLLING]
...
# Memory split (fit path): the 158 (S, T) f64 operator matrices are computed in
# two back-to-back segments so at most 87 are alive at once (was 158):
#   part1 = 9 K-line + 4 position + windows d in (5, 10)   (75 operators, cols 0..74)
#   part2 = windows d in (20, 30, 60)                      (87 operators, cols 75..157)
# Column order is unchanged: the part names are exactly ALPHA158[:75] /
# ALPHA158[75:]; each part recomputes the shared derived series (the
# "shared window statistics recomputed" cost of the split).
_PART1_WINS = (5, 10)
_PART2_WINS = (20, 30, 60)
ALPHA158_PART1_NAMES = (tuple(KLINE) + tuple(POS)
                        + tuple(f"{n}_{d}" for d in _PART1_WINS for n in ROLLING))
ALPHA158_PART2_NAMES = tuple(f"{n}_{d}" for d in _PART2_WINS for n in ROLLING)
assert list(ALPHA158_PART1_NAMES) + list(ALPHA158_PART2_NAMES) == ALPHA158


def _fill_k(out, O, H, L, C, VW):
    """The 13 K-line / position operators (verbatim from the original compute_158)."""
    HL = H - L
...
def _pre_series(C, V):
    """Derived series shared by the rolling operators (ret1, vlog, dc, dv, wser,
    P, logV1). Both parts recompute them through this single helper, so the two
    segments see bit-identical values."""
    T = C.shape[1]
    Cprev = _shift(C)
    ...
    return ret1, vlog, dc, dv, wser, P, logV1


def _fill_rolling(out, C, H, L, V, d, pre):
    """The 28 per-window operators for one window size d (verbatim from the
    original compute_158 d loop); writes out["<NAME>_%d" % d] for the 28 names."""
    ret1, vlog, dc, dv, wser, P, logV1 = pre
    T = C.shape[1]
    off = d - 1
    wC = _win(C, d)
    wH = _win(H, d)
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        out[f"MA_{d}"][:, off:] = meanC / C[:, off:]
        out[f"STD_{d}"][:, off:] = np.sqrt(varC) / C[:, off:]
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        out[f"BETA_{d}"][:, off:] = slope / C[:, off:]
        out[f"RESI_{d}"][:, off:] = ((C[:, off:] - meanC) - slope * (d - 1 - mean_t)) / C[:, off:]
    out[f"RSQR_{d}"][:, off:] = rsqr

    finC = _fin(wC)
    with np.errstate(divide="ignore", invalid="ignore"):
        out[f"MAX_{d}"][:, off:] = maxH / C[:, off:]
        out[f"MIN_{d}"][:, off:] = minL / C[:, off:]
        out[f"RSV_{d}"][:, off:] = (C[:, off:] - minL) / (maxH - minL + EPS)
        qtu = _quantile(wC, 0.8, d) / C[:, off:]
        qtd = _quantile(wC, 0.2, d) / C[:, off:]
    out[f"QTLU_{d}"][:, off:] = np.where(finC, qtu, np.nan)
    out[f"QTLD_{d}"][:, off:] = np.where(finC, qtd, np.nan)

    cl = C[:, off:][:, :, None]  # (S, T-d+1, 1) broadcasts over the window axis
    rank = ((wC < cl).sum(axis=2) + 0.5 * (wC == cl).sum(axis=2)) / d
    out[f"RANK_{d}"][:, off:] = np.where(finC, rank, np.nan)
    ...
    finH = _fin(wH)
    finL = _fin(wL)
    out[f"IMAX_{d}"][:, off:] = np.where(finH, amax, np.nan) / d
    out[f"IMIN_{d}"][:, off:] = np.where(finL, amin, np.nan) / d
    out[f"IMXD_{d}"][:, off:] = np.where(finH & finL, (amax - amin).astype(np.float64), np.nan) / d

    # CORR(C, log(V+1)) and CORD(C/C.shift(1), log(V/V.shift(1)+1))
    wy = _win(logV1, d)
    sy = wy.sum(axis=2)
    syy = (wy * wy).sum(axis=2)
    out[f"CORR_{d}"][:, off:] = _pearson(sumC, sy, sumC2, syy, (wC * wy).sum(axis=2), d)
    wx = _win(ret1, d)
    wx2 = _win(vlog, d)
    sx2 = wx.sum(axis=2)
    sy2 = wx2.sum(axis=2)
    out[f"CORD_{d}"][:, off:] = _pearson(
    ...
    out[f"CNTP_{d}"][:, off:] = cntp
    out[f"CNTN_{d}"][:, off:] = cntn
    out[f"CNTD_{d}"][:, off:] = cntp - cntn
    ...
    out[f"SUMP_{d}"][:, off:] = sump
    out[f"SUMN_{d}"][:, off:] = sumn
    out[f"SUMD_{d}"][:, off:] = sump - sumn
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        out[f"VMA_{d}"][:, off:] = meanV / (V[:, off:] + EPS)
        out[f"VSTD_{d}"][:, off:] = np.sqrt(varV) / (V[:, off:] + EPS)
    sw = _win(wser, d)
    sumw = sw.sum(axis=2)
    meanw = sumw / d
    varw = np.maximum((sw * sw).sum(axis=2) / d - meanw * meanw, 0.0)
    out[f"WVMA_{d}"][:, off:] = np.sqrt(varw) / (meanw + EPS)
    ...
    vsump = svp / (dva_ + EPS)
    vsumn = svn / (dva_ + EPS)
    out[f"VSUMP_{d}"][:, off:] = vsump
    out[f"VSUMN_{d}"][:, off:] = vsumn
    out[f"VSUMD_{d}"][:, off:] = vsump - vsumn


def compute_158_part1(W):
    """The 75 operator matrices for ALPHA158 columns 0..74 (9 K-line + 4 position
    + windows d in (5, 10), 29 each).

    W must contain (S, T) float64 arrays O, H, L, C (qfq prices), V (qfq volume),
    VWAP (= amount/vol, raw; the qfq factor cancels in the ratio). Returns dict
    name -> (S, T). The fit path calls part1/part2 back-to-back so at most 87 of
    the 158 (S, T) f64 matrices are alive at once."""
    O, H, L, C = W["O"], W["H"], W["L"], W["C"]
    V, VW = W["V"], W["VWAP"]
    S, T = C.shape
    out = {name: np.full((S, T), np.nan) for name in ALPHA158_PART1_NAMES}
    _fill_k(out, O, H, L, C, VW)
    pre = _pre_series(C, V)
    for d in _PART1_WINS:
        _fill_rolling(out, C, H, L, V, d, pre)
        with np.errstate(divide="ignore", invalid="ignore"):
            out[f"ROC_{d}"][:, d:] = C[:, :T - d] / C[:, d:]
    return out


def compute_158_part2(W):
    """The 87 operator matrices for ALPHA158 columns 75..157 (windows d in
    (20, 30, 60), 29 each). Same W contract as compute_158_part1."""
    H, L, C = W["H"], W["L"], W["C"]
    V = W["V"]
    S, T = C.shape
    out = {name: np.full((S, T), np.nan) for name in ALPHA158_PART2_NAMES}
    pre = _pre_series(C, V)
    for d in _PART2_WINS:
        _fill_rolling(out, C, H, L, V, d, pre)
        with np.errstate(divide="ignore", invalid="ignore"):
            out[f"ROC_{d}"][:, d:] = C[:, :T - d] / C[:, d:]
    return out
...
def compute_158(W):
    ...
    The fit path prefers compute_158_part1/_part2 back-to-back (bounds the live
    (S, T) f64 memory); this wrapper keeps the one-call interface (decision path)."""
    out = compute_158_part1(W)
    out.update(compute_158_part2(W))
    return out
