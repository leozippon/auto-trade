...
OVERLAYS = ("gp_overlay", "cma_overlay")
INDEX_BOOKS = OVERLAYS + ("noa_index",)

SHAPES = {
    "gp_overlay": {"cadence": "month", "seats_min": 80, "seats_max": 100, "max_replace": 10},
    "cma_overlay": {"cadence": "month", "seats_min": 80, "seats_max": 100, "max_replace": 10},
    "noa_index": {"cadence": "month", "seats_min": 12, "seats_max": 12, "max_replace": 2},
    "cashdiv_pool": {"cadence": "month", "seats_min": 12, "seats_max": 12, "max_replace": 2},
}
...
def _ranked(context, cross, daily, trading_days, shape, seats, seat_cash):
    ...
    if shape in INDEX_BOOKS:
        ...
        if shape == "gp_overlay":
            return composite.gp_overlay(context, pool, members, trading_days), section
        if shape == "cma_overlay":
            return composite.cma_overlay(context, pool, members, trading_days), section
        cheap = pool[pool["close"] * LOT <= seat_cash] if seat_cash > 0 else pool
        return composite.noa_index(context, cheap, members, trading_days, seats), section
    cheap = pool[pool["close"] * LOT <= seat_cash] if seat_cash > 0 else pool
    if cheap.empty:
        raise RuntimeError("no affordable tradable names for the 12-seat pool")
    composite.require_industry(cheap, "affordable pool")
    if shape != "cashdiv_pool":
        raise RuntimeError(f"unhandled BOOK {shape!r}")
    frame = composite.cashdiv_score(context, cheap)
...
def _book(ranked, keep, seats, seat_cash, shape):
    if shape in OVERLAYS or shape == "noa_index":
