...
- 每个决赛候选都要比过对照：等权或符号加权的基线、去掉登记机制的同一载体，以及 `run_null_control` 的随机同名组合对照；含可拟合参数的假设在 `fit` 里拟合而不是手调。对照同样走完整研究期验证，在 `batch_validate` 里登记为 `control: true`：它照常花 replay-year，但不计入冻结门的试验数，也永远不能被提名——把真候选登记成对照，它就失去了提名资格。
...
- 评分口径是主动序列（`acceptance_rules.graded_series`）：节点的日收益减去宿主零技能面板的合成收益。面板由宿主按该节点自己的成交骨架随机换名回放得到，衡量同一账户形状下不靠选股能拿到多少；每次正式验证都随结果给出 `benchmark.active_*` 与逐年的 `active_neutralized_excess_return`，策略除订单外不需要提供任何东西。
- 冻结门在提名时执行，规则与阈值在 `acceptance_rules.freeze_gate`：节点是完整研究期验证、指标有限、研究期权益回撤与主动回撤都不超过上限，本臂完整研究期验证数达到该节写明的下限，提名节点的主动信息比率、它去膨胀后的夏普概率（DSR）与主动超额为正的研究年数都过线；本臂设有跟踪授权（`tracking_mandate`）时，节点自身对本臂基准指数（事实里的 `benchmark_index`）的跟踪误差与市场 β 还要落在限内——那是约束，不是技能的证据。试验数是本臂验证过的全部非对照 revision（任何 span、任何尝试）加上各批 `offline_trials` 申报的离线筛选数；这些试验的日度评分序列彼此越相关，折算的有效试验数越少，而 DSR 的零技能离散度只由研究期长度决定，与本臂跑了哪些对照无关——每行 `selection_statistics.information_ratio_bar` 就是此刻过门需要的主动信息比率。`offline_trials` 必须如实申报：`offline_trials` = 本批申报前离线（研究期数据）筛过、读数影响了本批提交什么、但本批**没有**提交的候选配置数；提交进本批的候选在宿主验证时按 host trial 计，不重复申报；对照永不计；每个配置只在筛过之后第一个记下验证的批次申报一次；没有筛过任何候选配置才填 0。参考包的离线筛选门、参数网格、探针脚本都算筛过；少报就是替这次搜索瞒下了多重检验。不过门的提名被拒并给出命名原因与读数，会话继续；试验数只增不减，搜索越多，冻结需要的证据越强。
- 一条臂至多冻结一次，冻结后没有第二次机会：前推期不过，本臂就结束了。所以冻结的是逐年一致、比过对照、按毕业条件（`acceptance_rules.graduation`：前推期主动中性化超额的下界、近月窗口、两种回撤、成本压力、交易活跃度与跟踪授权）设计的节点。被冻结的是节点的不可变快照，工作副本不必先恢复到它。
...
```text
# 证据标准
- 整段指标与 `sub_windows` 的逐年行、原始超额、中性化超额与主动读数一起读：只靠一次风格暴露取得的优势不算边际，股票池本身在这段行情里的涨跌也不算——`benchmark.panel_neutralized_excess` 就是零技能在同一形状下拿到的那部分，主动读数已经把它减掉。组合构建与风险覆盖只有提高主动中性化超额或逐年一致性才算改进，只改善总收益或回撤的是风格暴露。证据接近时按逐年一致性与主动信息比率取舍，仍分不出则保留已验证版本。
- 「没有证明边际」的读数，任一成立即未证明：
  1. 主动中性化超额约为 0（年化回归截距，不与整段 `excess_return` 比大小，轻仓少成交时不可靠）；
  2. 半数以上研究年份的主动中性化超额为负，或优势只来自一个年份；
...
- 前推期能检出的边际有下限：`acceptance_rules.graduation.forward.minimum_detectable_excess` 与主动序列的残差跟踪误差成正比，主动跟踪误差更低的组合才让真实的边际被检出。
```
...
- `shell`：{"argv": ["python", "notes/probe.py"], "cwd": "."}——字符串数组直接执行，不经 shell；整行命令字符串按 `["bash", "-lc", "<该命令行>"]` 执行，管道、重定向、通配、`&&` 与 `$VAR` 照写生效；超过一行的脚本先 `write_file` 写成文件再按路径运行，不把脚本正文当参数传；只读信号筛选脚本 `/mnt/tools/screen.py` 不在任何读文件根内，只能这样经 `shell` 运行。
