...
_DIRECTIVE = (
    ...
    "在中性化超额与空对照分位两项上都严格高于它；单折比不过只记读数、锚点作父本，下一折先在新窗口复测同一候选对同一对照，"
    "复测前不做变体；连续两折比不过才关闭本机制并以 no_edge 携读数终止、不换家族，那说明一致预期缺口只是成长的另一种写法。"
...
_DEFENSIVE_DIRECTIVE = (
    ...
    "s1 与 s_lowvol 必须在中性化超额与空对照分位两项上都严格高于只用 20 日波动的 c_vol20，连续两折输了才关闭低波腿；"
    "s1 与 s_cfq 必须同样严格高于只用同比增速的 c_growth 和按盈利意外臂规则重建的 c_es，连续两折输了才关闭质量腿；"
    "单折输了只记读数、锚点作父本，下一折先在新窗口复测同一候选对同一对照，复测前不做变体；"
    "两腿都关即本臂以 no_edge 携读数终止、不换家族。s1 只有同时高于两条消融腿才可提名，否则提名更好的那条腿。"
