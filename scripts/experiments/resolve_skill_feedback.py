#!/usr/bin/env python3
"""Mark one Agent-filed skill feedback report resolved in its experiment's log.

The log is append-only: this writes a resolution line naming the report, the
outcome and a short note (name the edited entry or the commit when the outcome
is a library change), and never edits the report the session filed. The
console's 技能反馈 page then shows the report as resolved and drops it from the
default open list.
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--outcome",
        required=True,
        choices=list(SKILL_FEEDBACK_OUTCOMES),
        help=(
            "skill_updated names a curated-library edit; the other two close a "
            "report that needed none."
        ),
    )
    parser.add_argument(
        "--note", required=True, help="Short disposition, e.g. the edited entry."
    )
    ...
    try:
        record = append_skill_feedback_resolution(
    ...
    print(
        json.dumps(
            {"experiment": args.experiment, **record},
            ensure_ascii=False,
            sort_keys=True,
        )
    )
